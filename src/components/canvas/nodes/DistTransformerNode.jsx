  ...
  return (
    <group position={node.position} ref={groupRef} onClick={(e) => { e.stopPropagation(); selectNode(node.id); }}>
      ...
      <Billboard position={[0, 10.0, 0]}>
        ...
        <Text fontSize={0.27} color={satColor} fontWeight="bold" anchorX="center" anchorY="middle" position={[0, -0.86, 0]} outlineWidth={0.02} outlineColor="white">
          {`Load: ${node.load_saturation_percent?.toFixed(1)}%  T: ${node.ambient_temp_celsius?.toFixed(1)}°C`}
        </Text>
        <Text fontSize={0.25} color={lifespanColor} anchorX="center" anchorY="middle" position={[0, -1.24, 0]} outlineWidth={0.02} outlineColor="white">
